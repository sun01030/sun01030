...
REM Create SQL file with proper line breaks
(
...
REM Execute SQL file to create temp table, then export with \copy
echo Connecting to database...
(
    type %SQL_FILE%
    echo \copy temp_export TO STDOUT WITH CSV HEADER DELIMITER ',';
) | psql -h %PGSQL_HOST% -p %PGSQL_PORT% -U %PGSQL_USER% -d %PGSQL_DB% > %CSV_FILE% 2>>%LOG_FILE%
